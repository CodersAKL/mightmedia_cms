		...
		if ( sizeof( $sql ) > 0 ) {
			foreach ( $sql as $row ) {
				if ( kiek( "private_msg", "WHERE `to`=" . escape( $row['nick'] ) . "" ) < 51 ) {
					mysql_query1( "INSERT INTO `" . LENTELES_PRIESAGA . "private_msg` (`from` , `to` , `title` , `msg` , `date`) VALUES (" . escape(getSession('username')) . ", " . escape( $row['nick'] ) . ", '" . $lang['admin']['readme'] . "!', " . escape( $_POST['admin_chat'] ) . ", '" . time() . "')" );
				}
			}
		}
	}
...
if ( sizeof( $sql ) > 0 ) {
	...
	foreach ( $sql as $row ) {
		$text .= "
				<div class='" . ( is_int( $i / 2 ) ? 'tr2' : 'tr' ) . "'><em><a href=\"" . url( "d," . $row['id'] . "" ) . "\" onClick=\"return confirm('" . $lang['admin']['delete'] . "?')\">[{$lang['admin']['delete']}]</a> " . ( (getSession('username') == $row['admin']) ? "<a href=\"" . url( "r," . $row['id'] . "" ) . "\">[{$lang['admin']['edit']}]</a> " : "" ) . $row['admin'] . " [" . date( 'Y-m-d H:i:s ', $row['date'] ) . "] - " . kada( date( 'Y-m-d H:i:s ', $row['date'] ) ) . " " . naujas( $row['date'], $row['admin'] ) . "</em><br />
