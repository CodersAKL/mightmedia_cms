...
if ( isset( $_POST['admin_chat_send'] ) && $_POST['admin_chat_send'] == getLangText('admin', 'send') && !empty( $_POST['admin_chat'] ) ) {
		...
		if ( sizeof( $sql ) > 0 ) {
			foreach ( $sql as $row ) {
				if ( kiek( "private_msg", "WHERE `to`=" . escape( $row['nick'] ) . "" ) < 51 ) {
					mysql_query1( "INSERT INTO `" . LENTELES_PRIESAGA . "private_msg` (`from` , `to` , `title` , `msg` , `date`) VALUES (" . escape(getSession('username')) . ", " . escape( $row['nick'] ) . ", '" . getLangText('admin', 'readme') . "!', " . escape( $_POST['admin_chat'] ) . ", '" . time() . "')" );
				}
			}
		}
	}
	...
	mysql_query1( "INSERT INTO `" . LENTELES_PRIESAGA . "admin_chat` (admin, msg, date) VALUES(" . escape(getSession('username')) . "," . escape( $extra . $_POST['admin_chat'] ) . ",'" . time() . "')" ) or die( mysqli_error($prisijungimas_prie_mysql) );
...
if ( isset( $url['r'] ) && !isset( $url['d'] ) && !isset( $url['a'] ) && isnum( $url['r'] ) && $url['r'] > 0 ) {
	if ( !isset( $_POST['admin_chat_send'] ) ) {
		...
		mysqli_query( "UPDATE `" . LENTELES_PRIESAGA . "admin_chat` SET `msg`=" . escape( $_POST['admin_chat'] ) . ",`date` = '" . time() . "' WHERE `admin`=" . escape(getSession('username')) . " AND id=" . escape( (int)$url['r'] ) . " LIMIT 1" );
...
if ( sizeof( $sql ) > 0 ) {
	...
	foreach ( $sql as $row ) {
		$text .= "
				<div class='" . ( is_int( $i / 2 ) ? 'tr2' : 'tr' ) . "'>
					<em>
						<a href=\"" . url( "d," . $row['id'] . "" ) . "\" onClick=\"return confirm('" . getLangText('admin', 'delete') . "?')\">[" . getLangText('admin',  'delete') . "]</a>
						" . ( (getSession('username') == $row['admin']) ? "<a href=\"" . url( "r," . $row['id'] . "" ) . "\">[" . getLangText('admin',  'edit') . "]</a> " : "" ) . $row['admin'] . " [" . date( 'Y-m-d H:i:s ', $row['date'] ) . "] - " . kada( date( 'Y-m-d H:i:s ', $row['date'] ) ) . " " . naujas( $row['date'], $row['admin'] ) . "
					</em><br />
					" . bbcode( $row['msg'] ) . "<br />
				</div>";
		$i++;
