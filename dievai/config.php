...
if (isset($_POST) && !empty($_POST) && isset($_POST['Konfiguracija'])) {
		$q = array();
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape($_POST['Apie']) . ",'Apie')  ON DUPLICATE KEY UPDATE val=" . escape($_POST['Apie']);
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape(input(strip_tags($_POST['Keywords']))) . ",'Keywords')  ON DUPLICATE KEY UPDATE val=" . escape(input(strip_tags($_POST['Keywords'])));
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape(input(strip_tags($_POST['Pavadinimas']))) . ",'Pavadinimas')  ON DUPLICATE KEY UPDATE val=" . escape(input(strip_tags($_POST['Pavadinimas'])));
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape($_POST['Copyright']) . ",'Copyright')  ON DUPLICATE KEY UPDATE val=" . escape($_POST['Copyright']);
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape(input(strip_tags($_POST['Pastas']))) . ",'Pastas')  ON DUPLICATE KEY UPDATE val=" . escape(input(strip_tags($_POST['Pastas'])));
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape((int)$_POST['Palaikymas']) . ",'Palaikymas')  ON DUPLICATE KEY UPDATE val=" . escape((int)$_POST['Palaikymas']);
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape((int)$_POST['News_limit']) . ",'News_limit')  ON DUPLICATE KEY UPDATE val=" . escape((int)$_POST['News_limit']);
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape($_POST['Maintenance']). ",'Maintenance')  ON DUPLICATE KEY UPDATE val=" . escape($_POST['Maintenance']);
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape(input(strip_tags($_POST['Stilius']))) . ",'Stilius')  ON DUPLICATE KEY UPDATE val=" . escape(input(strip_tags($_POST['Stilius'])));
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape(basename($_POST['pirminis'],'.php')) . ",'pirminis')  ON DUPLICATE KEY UPDATE val=" . escape(basename($_POST['pirminis'],'.php'));
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape(basename($_POST['kalba'],'.php')) . ",'kalba')  ON DUPLICATE KEY UPDATE val=" . escape(basename($_POST['kalba'],'.php'));
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape((int)$_POST['keshas']) . ",'keshas')  ON DUPLICATE KEY UPDATE val=" . escape((int)$_POST['keshas']);
	$q[] = "INSERT INTO table `" . LENTELES_PRIESAGA . "nustatymai` (val,key) VALUES (" . escape((int)$_POST['koment']) . ",'kmomentarai_sveciams')  ON DUPLICATE KEY UPDATE val=" . escape((int)$_POST['koment']);
	foreach ($q as $sql) {
		mysql_query1($sql);
	}
...
foreach ($kalbos as $file) {
	if ($file['type'] == 'file') {
		$kalba[basename($file['name'])] = strtoupper(basename($file['name'], '.php'));
	}
}
...
$nustatymai = array("Form" => array("action" => "", "method" => "post", "enctype" => "", "id" => "", "class" => "", "name" => "reg"), 
	...
	"{$lang['admin']['newsperpage']}:" => array("type" => "text", "value" => input($conf['News_limit']), "name" => "News_limit", 'extra' => "onkeyup=\"javascript:this.value=this.value.replace(/[^0-9]/g, '');\"", "class" => "select"), 
