...
if (!defined("OK") || ! ar_admin(basename(__file__))) {
    header('location: http://' . $_SERVER["HTTP_HOST"]);	//header kadangi neaisku ar funkcijos inkludintos ar ne - todel naudojam default.
	die();	//die() - nes nesvarbu ar yra header peradresavimas ar ne, skriptas toliau vykdomas - todel turim nutraukti procesa su komanda die();
}

//Duodam tik 5mb uploadinimui
...
if (isset($_GET['v'])) {
    ...
    if ($_GET['v'] == 8) {
        ...
        $text .= "</ul></fieldset>";
        $limit = 10;
        $sql2 = mysql_query1("SELECT * FROM  `" . LENTELES_PRIESAGA . "galerija` WHERE `lang` = " . escape(lang()) . " AND `categorija`=" . escape((isset($_GET['k']) ? $_GET['k'] : 0)) . "  LIMIT $p,$limit");
            ...
            if (sizeof($q) > 0) {
                foreach ($q as $row) {
                    ...
                    $info[] = array(//"ID"=> $row['ID'],
                        $lang['admin']['gallery_image'] => "<a href='" . url("?id,{$_GET['id']};a,{$_GET['a']}") . "' title='<img src=" . ROOT . "images/galerija/" . $row['file'] . "><br /><b>{$lang['admin']['gallery_author']}:</b> " . $autorius . "<br /><b>{$lang['admin']['gallery_date']}:</b> " . date('Y-m-d H:i:s ', $row['data']) . "<br /><b>{$lang['admin']['gallery_about']}:</b> " . $row['apie'] . "'>" . $row['pavadinimas'] . " ...</a>",
