...
if (isset($_GET) && !empty($_GET)) {
    ...
    if (isset($_GET['nick']) && !empty($_GET['nick'])) {
        if (preg_match('/[^A-Za-z0-9]/', $_GET['nick'])) {
            ...
            if (isset($vardas)) {
                $einfo = mysql_num_rows(mysql_query("SELECT `nick` FROM `" . LENTELES_PRIESAGA .
                    "users` WHERE `nick`=" . escape($vardas) . " LIMIT 1"));
                if ($einfo != 0) {
        ...
        if (isset($email)) {
            $einfo = mysql_num_rows(mysql_query("SELECT `email` FROM `" . LENTELES_PRIESAGA .
                "users` WHERE `email`=" . escape($email) . " LIMIT 1"));
            if ($einfo != 0) {
        ...
        if (count($files) > 2) {
            ...
            foreach ($files as $file) {
                if (file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' &&
                    ...
                    echo "<li class=\"directory collapsed\"><a href=\"#\" rel=\"" . htmlentities($_POST['dir'] .
            ...
            foreach ($files as $file) {
                if (file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' &&
                    ...
                    echo "<li class=\"file ext_$ext\"><a href=\"#\" rel=\"" . htmlentities($_POST['dir'] .
                        $file) . "\">" . htmlentities($file) . "</a></li>";
                }
            }
            echo "</ul>";
