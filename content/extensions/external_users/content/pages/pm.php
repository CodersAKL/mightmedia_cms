...
if ( isset( $url['v'] ) ) {
	if ( !empty( $url['v'] ) && (int)$url['v'] > 0 && isnum( $url['v'] ) ) {
		...
		if ( $sql ) {
			...
			$laiskas = "
				<div class=\"pm_read\"><b>" . getLangText('user',  'pm_from') . ":</b>  " . input( $sql['from'] ) . "<br><b>" . getLangText('user',  'pm_to') . ":</b> " . $sql['to'] . "<br /> <b>" . getLangText('user',  'pm_subject') . ":</b> " . ( isset( $sql['title'] ) && !empty( $sql['title'] ) ? input( $sql['title'] ) : getLangText('user', 'pm_nosubject')) . "<br><br><b>" . getLangText('user',  'pm_message') . ":</b><br />" . bbcode( $sql['msg'] ) . "<br /><br /></div>
				" . ( strtolower( $sql['to'] ) == strtolower( $_SESSION[SLAPTAS]['username'] ) ? "<form name=\"replay_pm\" action='" . url( "?id," . $conf['pages']['pm.php']['id'] . ";n,1;u," . str_replace( "=", "", base64_encode( $sql['from'] ) ) . ";i," . $url['v'] ) . "' method=\"post\">
					<input type=\"submit\" value=\"" . getLangText('user',  'pm_reply') . "\"/> <input type=\"button\" value=\"" . getLangText('user',  'pm_delete') . "\" onclick=\"location.href='" . url( "d," . $url['v'] . ";v,0" ) . "'\"/>
				</form>" : "" ) . "
				";
