...
HTML;
	if ( puslapis( 'reg.php' ) ) {
		$text .= "<a href=\"" . url( "?id," . $conf['pages']['reg.php']['id'] ) . "\">" . getLangText('user',  'registration') . "</a> ";
	}
	if ( puslapis( 'slaptazodzio_priminimas.php' ) ) {
		$text .= "<a href=\"" . url( "?id," . $conf['pages']['slaptazodzio_priminimas.php']['id'] ) . "\">" . getLangText('user',  'pass_forget') . "</a>";
		$title = getLangText('user', 'for_members');
	}
} else {
	...
	if ( isset( $conf['pages']['edit_user.php']['id'] ) ) {
		$text .= "<li><a href=\"" . url( "?id," . $conf['pages']['edit_user.php']['id'] ) . "\"><img src=\"core/assets/images/icons/user_edit.png\" alt=\"@\" style=\"vertical-align: middle;\" border=\"0\" /> " . getLangText('user',  'edit_profile') . "</a></li>";
	}
	//Pm nuoroda
	if ( puslapis( 'pm.php' ) ) {
