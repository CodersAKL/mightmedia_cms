...
if ( $tid > 0 && $sid > 0 && $kid == 0 && $lid == 0 && $rid == 0 && $aid == 0 ) {
	...
	if ( isset( $teises['teises'] ) && teises( unserialize( $teises['teises'] ), getSession('level'))) {
		...
		if ( !empty( $straipsnis ) ) {
			...
			$limit  = 15;
			$gaunam = mysql_query1( 
				"SELECT `" . LENTELES_PRIESAGA . "users.nick`, `" . LENTELES_PRIESAGA . "users.taskai`, 
				`" . LENTELES_PRIESAGA . "users.levelis`, `" . LENTELES_PRIESAGA . "users.gim_data`, `" . LENTELES_PRIESAGA . "users.email`, 
				`" . LENTELES_PRIESAGA . "users.id`, `" . LENTELES_PRIESAGA . "users.miestas`, `" . LENTELES_PRIESAGA . "users.icq`, 
				`" . LENTELES_PRIESAGA . "users.msn`, `" . LENTELES_PRIESAGA . "users.skype`, `" . LENTELES_PRIESAGA . "users.aim`, `" . LENTELES_PRIESAGA . "users.url`, 
				`" . LENTELES_PRIESAGA . "users.parasas`, `" . LENTELES_PRIESAGA . "users.forum_atsakyta`, `" . LENTELES_PRIESAGA . "d_zinute.id` AS `zid`, 
				`" . LENTELES_PRIESAGA . "d_zinute.nick` AS `nikas`, `tid`, `sid`,`zinute`,`laikas` FROM `" . LENTELES_PRIESAGA . "users` INNER 
				JOIN `" . LENTELES_PRIESAGA . "d_zinute` 
				ON " . LENTELES_PRIESAGA . "d_zinute.nick=" . LENTELES_PRIESAGA . "users.id WHERE `sid`='" . $tid . "' ORDER BY laikas ASC LIMIT {$pid},{$limit}" );
			...
			foreach ( $gaunam as $row ) {
				...
				$reply = (getSession('level') > 0 ? ' <a style="float: right; margin-right:2px;"  href="' . url( "q," . $row['zid'] . "#end" ) . '" title="' . getLangText('admin', 'pm_reply') . '"><img src="' . $imagedir . lang() . '/atsakyti.png" border="0" alt="re"></a> ' : '' );
			...
			if (! empty(getSession('id')) && $kid == 0 && $lid == 0 && $rid == 0 && $eid > 0 ) {
				...
				if ( !empty( $_POST['msg'] ) && $_POST['action'] == 'f_update' ) {
					if ( ar_admin( 'frm' ) ) {
						mysql_query1( "UPDATE `" . LENTELES_PRIESAGA . "d_zinute` SET `zinute`=" . escape( $_POST['msg'] . "\n[sm][i]" . getLangText('forum',  'edited_by') . ": " . getSession('username') . " " . date( 'Y-m-d H:i:s', time() ) . "[/i][/sm]" ) . " WHERE `id`=" . escape( $eid ) );
					} else {
						mysql_query1( "UPDATE `" . LENTELES_PRIESAGA . "d_zinute` SET `zinute`=" . escape( $_POST['msg'] . "\n[sm][i]" . getLangText('forum',  'edited_by') . ": " . getSession('username') . " " . date( 'Y-m-d H:i:s', time() ) . "[/i][/sm]" ) . " WHERE `id`=" . escape( $eid ) . " AND `nick`=" . escape(getSession('id')) );
					}
...
$.post('core/assets/javascript/forum/preview.php', {'msg':$('textarea#msg').val()}, function(data) {
