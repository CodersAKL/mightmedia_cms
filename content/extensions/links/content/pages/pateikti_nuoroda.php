 ...
 **/
if ( isset( $_POST['action'] ) && $_POST['action'] == getLangText('admin', 'links_create') ) {
	...
	if ( !preg_match( $pattern, $url ) ) {
		klaida( getLangText('system', 'error'), getLangText('admin', 'links_bad') );
	} else {
	...
	`lang` = " . escape( lang() ) . "" );
		...
		if ( $result ) {
			msg( getLangText('system', 'done'), getLangText('admin', 'links_sent') . "." );
		} else {
			klaida( getLangText('system', 'error'), getLangText('admin', 'links_allfields') . "." );
		}
	}
...
$links = array( "Form"                        => array( "action" => url( "?id," . $conf['pages'][basename( __file__ )]['id'] ), "method" => "post", "name" => "reg" ),
                   getLangText('system', 'category')   => array( "type" => "select", "value" => $categories, "name" => "kat" ),
                   getLangText('admin', 'links_title') => array( "type" => "text", "value" => "", "name" => "name" ),
