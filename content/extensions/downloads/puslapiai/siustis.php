			...
			if ( $sql['path'] == $k ) {
				...
				$info[] = array(
					getLangText('system', 'categories') => "
						<a style=\"float: left;\" class=\"kat\" href='" . url( "?id," . $url['id'] . ";k," . $sql['id'] . "" ) . "'>
							<img src='core/assets/images/naujienu_kat/" . input( $sql['pav'] ) . "' alt=\"\"  border=\"0\" />
						</a>
						<div>
							<a href='" . url( "?id," . $url['id'] . ";k," . $sql['id'] . "" ) . "'><b>" . input( $sql['pavadinimas'] ) . "</b></a>
							<span class=\"small_about\"style='font-size:9px;width:auto;display:block;'>
								<div>" . input( $sql['aprasymas'] ) . "</div>
								<div>" . getLangText('category',  'downloads') . ": " . $sqlkiek . "</div>
							</span>
						</div>" //,
				);
			}
  ...
  LIMIT 1", 86400 );
	...
	if ( sizeof( $sql ) > 0 && isset( $sql['id'] ) ) {
		...
		if ( !isset( $sql['teises'] ) || teises( $sql['teises'], $_SESSION[SLAPTAS]['level'] ) ) {
			...
			lentele( 
				getLangText('download', 'downloads') . " >> " . ( isset( $sql['Kategorija'] ) ? input( $sql['Kategorija'] ) . " >> " : "" ) . input( $sql['pavadinimas'] ) . "", $ble->render( $info2 ) . 
				"<a href=\"javascript: history.go(-1)\">" . getLangText('download',  'back') . "</a>" );
