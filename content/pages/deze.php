...
if ( sizeof( $sql2 ) > 0 ) {
	...
	foreach ( $sql2 as $row ) {
		...
		$tr = $i % 2 ? '2' : '';
		if ( ar_admin( 'com' ) ) {
			$admin_tools = "<span style=\"float: right;\"><a href='" . url( "d," . $row['id'] . "" ) . "' onclick=\"return confirm('{$lang['system']['delete_confirm']}') \"><img height=\"15\" src='core/assets/images/icons/cross.png' alt='[{$lang['admin']['delete'] }]' title='{$lang['admin']['delete'] }' class='middle' border='0' /></a>
			<a href='" . url( "r," . $row['id'] . "" ) . "'><img height=\"15\" src='core/assets/images/icons/pencil.png' alt='[{$lang['admin']['edit'] }]' title='{$lang['admin']['edit'] }' class='middle' border='0' /></a> </span>";
		}
