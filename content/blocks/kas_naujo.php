	...
	if ( sizeof( $q ) > 0 ) {
		...
		foreach ( $q as $row ) {
			...
			if ( $row['pid'] == 'content/pages/naujienos' && isset( $conf['pages']['naujienos.php']['id'] ) ) {
				...
				//Strapsniai
			} elseif ( $row['pid'] == 'content/pages/straipsnis' && isset( $conf['pages']['straipsnis.php']['id'] ) ) {
				$link = "m," . $row['kid'];
				//Galerija
			} elseif ( $row['pid'] == 'content/pages/galerija' && isset( $conf['pages']['galerija.php']['id'] ) ) {
				$link = "m," . $row['kid'];
				//Vartotojai
			} elseif ( $row['pid'] == 'content/pages/view_user' && isset( $conf['pages']['view_user.php']['id'] ) ) {
				$narys = mysql_query1( "SELECT * FROM `" . LENTELES_PRIESAGA . "users` WHERE `id`=" . escape( $row['kid'] ) . " LIMIT 1" );
				$link  = "" . $narys['nick'];
				//Kas naujo
			} elseif ( $row['pid'] == 'content/pages/todo' && isset( $conf['pages']['todo.php']['id'] ) ) {
				$link = "v," . $row['kid'];
				//Kodu
			} elseif ( $row['pid'] == 'content/pages/codebin' && isset( $conf['pages']['codebin.php']['id'] ) ) {
				$link = "c," . $row['kid'];
				//Balsavimo archyvas
			} elseif ( $row['pid'] == 'content/pages/blsavimo_archyvas' && isset( $conf['pages']['blsavimo_archyvas.php']['id'] ) ) {
				$link = "m," . $row['kid'];
			}

			$file = str_replace( 'content/pages/', '', $row['pid'] );
