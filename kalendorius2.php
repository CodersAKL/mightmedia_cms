...
//Gauname šiandienos (mėnesis-diena)
	...
	if (array_key_exists($siandien, $array)) {
		foreach ($array[$siandien] as $key => $val) {
			if (empty($return)) {	//Jei išvedam šventę pirmą kartą
				$return .= $val;
			}	
			else {
				$return .= ", <br />" . $val;
...
$sventes = array(
	 ...
	 "8-15" => array("Žolines"),
	 "11-1" => array("Visu šventuju diena"),
	 "12-25" => array("Kaledos"),
	 ...
	 "9-1" => array("Mokslo ir žiniu diena"),
	 "9-8" => array("Šiline (Švc. Mergeles Marijos gimimo diena)", "Vytauto Didžiojo karunavimo diena")
 );
