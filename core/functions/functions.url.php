	...
	function url( $str ) {
		...
		if ( substr( $str, 0, 1 ) == '?' ) {
			...
			if ( !empty( $conf['F_urls'] ) && $conf['F_urls'] != '0' ) {
				...
				$return = url() . ROOT . $url_title . implode( ( $conf['F_urls'] != '0' ? $conf['F_urls'] : ';' ), $linkai );
			} else {

				$return = url() . ( substr( $str, 4, 3 ) == '999' && ( empty( $conf['F_urls'] ) || $conf['F_urls'] == '0' ) ? 'main.php' : ( substr( $str, 0, 1 ) != '?' ? '' : ROOT ) ) . $str;
			}
		} else {
