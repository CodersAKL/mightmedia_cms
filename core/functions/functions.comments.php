...
function comments($id, $hide = false) {
	...
	if ( $conf['kmomentarai_sveciams'] != 3 ) {
		...
		if ( isset( $_POST['n_kom'] ) && !empty( $_POST['n_kom'] ) && !empty( $_POST['Naujas'] ) && $_POST['Naujas'] == $lang['comments']['send'] && isset( $_POST['id'] ) && !empty( $_POST['id'] ) && (! empty(getSession('id')) || $conf['kmomentarai_sveciams'] == 1 ) ) {
			if ( ( isset( $_POST['code'] ) && strtoupper( $_POST['code'] ) == getSession('code')) || ! emoty(getSession('id'))) {
				...
				$nick    = (! empty(getSession('username')) ? getSession('username') : ( !empty( $_POST['name'] ) ? serialize( array( trimlink( strip_tags( $_POST['name'] ), 9 ), getip() ) ) : serialize( array( $lang['system']['guest'], getip() ) ) ) );
