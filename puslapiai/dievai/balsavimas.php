...
if (empty($url['v']))
{
    ...
    $is = mysql_query1("SELECT * FROM `" . LENTELES_PRIESAGA . "balsavimas` WHERE `ijungtas`='TAIP'  ORDER BY `id` DESC LIMIT 1 ") or die(mysql_error());
    $sql = mysql_fetch_assoc($is);
...
{
    $result = mysql_query1("DELETE FROM `" . LENTELES_PRIESAGA . "balsavimas` WHERE `id`= " . escape((int)$_POST['id']) . " LIMIT 1") or die(mysql_error());
    header("Location: ?id," . $_GET['id'] . ";a," . $_GET['a']);
}
if (isset($_POST['b_edit']) && $_POST['b_edit'] == $lang['admin']['edit'])
{
    $result2 = mysql_query1("UPDATE `" . LENTELES_PRIESAGA . "balsavimas` SET info='" . $_POST['leid'] . "', `ijungtas` = " . escape($_POST['ar']) . " WHERE `id`='" . $url['n'] . "' LIMIT 1 ;") or die(mysql_error());
    header("Location: ?id," . $_GET['id'] . ";a," . $_GET['a']);
}
...
{
    $kl = $_POST['b_kl'];
    $ats1 = (isset($_POST[0]) && !empty($_POST[0]) ? strip_tags($_POST[0]) . ';0' : ';0');
    $ats2 = (isset($_POST[1]) && !empty($_POST[1]) ? strip_tags($_POST[1]) . ';0' : ';0');
    $ats3 = (isset($_POST[2]) && !empty($_POST[2]) ? strip_tags($_POST[2]) . ';0' : ';0');
    $ats4 = (isset($_POST[3]) && !empty($_POST[3]) ? strip_tags($_POST[3]) . ';0' : ';0');
    $ats5 = (isset($_POST[4]) && !empty($_POST[4]) ? strip_tags($_POST[4]) . ';0' : ';0');

    $result = mysql_query1("INSERT INTO `" . LENTELES_PRIESAGA . "balsavimas` (`info`, `autorius`, `laikas`, `klausimas`, `pirmas`, `antras`, `trecias`, `ketvirtas`,`penktas`) VALUES ('" . $_POST['leid'] . "', '" . $_SESSION['id'] . "', '" . time() . "','" . $kl . "','" . $ats1 . "','" . $ats2 . "','" . $ats3 . "','" . $ats4 . "','" . $ats5 . "')") or die(klaida("Klaida",
        mysql_error()));
    if ($result)
    {
