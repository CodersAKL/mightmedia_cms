...
{
    $text .= "
				<div class='title'><em><a href=\"" . url("d," . $row['id'] . "") . "\" >[{$lang['admin']['delete']}]</a> " . (($_SESSION['username'] == $row['admin']) ? "<a href=\"" . url("r," . $row['id'] . "") . "\">[{$lang['admin']['edit']}]</a> " : "") . $row['admin'] . " [" . date('Y-m-d H:i:s ', $row['date']) . "] - " . kada(date('Y-m-d H:i:s ', $row['date'])) . " " . naujas($row['date'],
        $row['admin']) . "</em></div>
