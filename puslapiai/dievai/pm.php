...
if (isset($url['v'])) {
	if (!empty($url['v']) && (int)$url['v'] > 0) {
		...
		if (count($sql) > 0) {
			$laiskas = "
				<b>{$lang['admin']['pm_sender']}:</b>  " . $sql['from'] . "<br><b>{$lang['admin']['pm_reciever']}:</b> " . $sql['to'] . "<br> <b>{$lang['admin']['pm_subject']}:</b> " . (isset($sql['title']) && !empty($sql['title']) ? input(trimlink($sql['title'], 40)) : $lang['admin']['pm_nosubject']) . "<br><br><b>{$lang['admin']['pm_message']}:</b><br>" . bbcode(wrap($sql['msg'], 40)) . "<br><br>
...
if (sizeof($sql) > 0) {
	foreach ($sql as $row) {
		...
		$info[] = array("#" => $extra, "{$lang['admin']['pm_sender']}" => user($row['from_nick'], $row['from_id']), "{$lang['admin']['pm_reciever']}" => user($row['to_nick'], $row['to_id']), "{$lang['admin']['pm_subject'] }" => "<a href=\"?id,999;a," . $_GET['a'] . ";v," . $row['id'] . "\" title=\"<b>Laiško ištrauka:</b> " . input(trim(strip_tags(str_replace(array('[', ']'), '', $row['msg'])))) . "...\" style=\"display:block\">" . (isset($row['title']) && !empty($row['title']) ? trimlink(input($row['title']), 10) : 'Be temos') . "</a>", //"Pavadinimas"=>"<a href=\"?id,46;a,20;v,".$row['id']."\" title=\"header=[Laiško ištrauka:] body=[".input(bbcode($sql['msg']))."...] fade=[on]\">".input($row['title'])."</a>",
