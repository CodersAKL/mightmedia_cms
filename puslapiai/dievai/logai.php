...
if (isset($url['d']) && isnum($url['d']) && LEVEL == 1) {
    ...
    $sql = mysql_fetch_assoc(mysql_query1("SELECT id, INET_NTOA(ip) AS ip, action, time FROM `" . LENTELES_PRIESAGA . "logai` WHERE id=" . escape($url['v']) . " LIMIT 1"));
...
if (!empty($url['t'])) {
    ...
    if (mysql_num_rows($sql) > 0) {
        ...
        $nustatymai = array("Form" => array("action" => "?id," . $url['id'] . ";a," . $url['a'] . ";d,0", "method" => "post", "enctype" => "", "id" => "", "class" => "", "name" => "reg"), "{$lang['admin']['logs_deletebyip']}:" => array("type" => "select", "value" => $select, "selected" => ip2long($_SERVER['REMOTE_ADDR']), "name" => "ip"), "" => array("type" => "submit", "name" =>
            "del_all", "value" => $lang['admin']['delete']));
        $bla = new forma();
        lentele("{$lang['admin']['logs_deletebyip']}", $bla->form($nustatymai));
        
        $nustatymai = array(
				"Form"=>array("action"=>"?id,".$url['id'].";a,".$url['a'].";t,1","method"=>"post","enctype"=>"","id"=>"","class"=>"","name"=>"reg"),
				$lang['admin']['logs_clear']=>array("type"=>"submit","name"=>"del_all","value"=>"Valyti","extra"=>"onclick=\"return confirm('{$lang['system']['delete_confirm']}')\"")
        );
        $bla = new forma();
