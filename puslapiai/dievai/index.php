...
$text = "<table border=\"0\"><tr><td><div>\n";
foreach($glob as $id => $file) {
	$file = basename($file,'.php');
	$admin_pages[$id] = $file;
	if ((isset($conf['puslapiai'][$file.'.php']['id']) || in_array($file, array('config','meniu','logai','paneles','vartotojai','komentarai'))) && !in_array($file, array('index','pokalbiai'))) {
		$text .= "<div class=\"blokas\"><center><a href=\"?id," . $url['id'] . ";a,{$id}\"><img src=\"images/admin/{$file}.png\" alt=\"\$lang['admin']['{$file}']\" title=\"\$lang['admin']['{$file}']\" />{$lang['admin'][$file]}</a></center></div>";
	}
}
