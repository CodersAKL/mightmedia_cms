...
if (sizeof($sql2) > 0) {	
	foreach ($sql2 as $row) {
		...
		if (defined("LEVEL") && LEVEL == 1) {
			...
			$tr = "";
		$text .= "<div class=\"tr$tr\"><em><a href=\"?id," . $url['id'] . ";p,$p#" . $row['id'] . "\" name=\"" . $row['id'] . "\" id=\"" . $row['id'] . "\"><img src=\"images/icons/bullet_black.png\" alt=\"#\" class=\"middle\" border=\"0\" /></a> " .$row['nikas']. " $extra (" . date('Y-m-d H:i:s',$row['time']) . ") - " . kada(date('Y-m-d H:i:s',$row['time'])) . "</em><br />" . smile(bbchat($row['msg'])) . "</div>";
