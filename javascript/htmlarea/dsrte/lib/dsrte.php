...
$dir="javascript/htmlarea/dsrte/";
define("DIR",$dir);
// Base classes
require_once (''.$dir.'lib/dsrte.command.php');
...
class dsRTE extends dsRTECommand
{
    ...
    public function getHTML( $default_text = '' )
    {
        ...
        if ( $this->resizable )
            $html .= '<tr><td style="border-top:1px solid #cccccc"><div class="rte-resize" id="'.$this->id.'-resize"><img alt="" src="'.DIR.'/images/resize.gif" /></div></td></tr>';
        ...
        $html .= '<textarea rows="1" cols="1" id="'.$this->id.'-ta" name="'.$this->id.'" style="display:none">'.$default_text.'</textarea>';
    ...
    public function getScripts()
    {
        ...
        if ( self::$notAddedYet )
        {
            ...
            $scripts[] = '<script type="text/javascript" src="'.DIR.'lib/dsrte.js"></script>';

            // check for resizable editor
            if ( $this->resizable )
                $scripts[] = '<script type="text/javascript" src="'.DIR.'lib/dsrte.resizer.js"></script>';
...
foreach ( glob( ''.DIR.'lib/plugins/*.php' ) as $plugin )
    require_once( $plugin );
