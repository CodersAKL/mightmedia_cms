...
<select>\
	...
	table			: {exec: lwrte_table, tags: ['table'] },
...
function lwrte_style_init(rte) {
	...
	if(rte.css.length) {	
		$.ajax({
			url: "javascript/htmlarea/jquery.rte1_2/styles.php", 
			type: "POST",
...
<p class="submit"><button id="ok">Ok</button><button id="cancel">Cancel</button></p>'
	...
	var upload = $('#file', panel).upload( {
		autoSubmit: false,
		action: 'javascript/htmlarea/jquery.rte1_2/uploader.php',
		onSelect: function() {
...
function lwrte_table() {
	var self = this;
	var panel = self.create_panel('Insert table', 385);
	panel.append('\
<p><label>Rows</label><input type="text" id="rows" size="30" value=""></p><p><label>Cols</label><input type="text" id="cols" size="30" value=""></p><p><label>Border</label><input type="text" id="border" size="30" value=""></p>\
<div class="clear"></div>\
<p class="submit"><button id="ok">Ok</button><button id="cancel">Cancel</button></p>'
).show();
	...
	$('#ok', panel).click( 
		function() {
			...
			var col = cols.val();
			var bord = border.val();
			var tmp = '<table border='+parseInt(bord)+'><tr>';
			var num =0;
			var kiek = col*row;
while(kiek!=num){
num++;
tmp = tmp+'<td>x</td>';		
if(typeof(num/col)=='number' && parseInt(num/col)==num/col){ tmp = tmp+'</tr>';
if(kiek-num>1){tmp = tmp+'<tr>';}
}


}
tmp=tmp+'</table>';
			self.editor_cmd('insertHtml', tmp);
			panel.remove(); 
			return false;
		}
	)
