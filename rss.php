...
<rss version="2.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:atom="http://www.w3.org/2005/Atom">
 ...
 <image>
			...
			$result = mysql_query1("SELECT SQL_CACHE * FROM `" . LENTELES_PRIESAGA . "naujienos`	WHERE `rodoma`= 'TAIP' AND `lang` = ".escape(basename($_GET['lang'],'.php'))." ORDER BY `data` DESC LIMIT 50");

			//Iterate over the rows to create each item
			//<image>'.adresas()."images/naujienu_kat/".$kategorija['pav'].'</image>
			foreach($result as $row) {
				$kategorija = mysql_query1("SELECT  SQL_CACHE * FROM `" . LENTELES_PRIESAGA . "grupes` where `id`=".escape($row['kategorija'])." AND `lang` = ".escape(basename($_GET['lang'],'.php'))." LIMIT 1",60);
